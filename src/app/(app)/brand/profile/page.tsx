...
export default function ProfilePage() {
    ...
    const { profile, isLoading, error, refetch, updateProfile } = useProfileData();
    ...
    const handleGenerateBio = async () => {
        ...
        try {
            ...
            if (result.message === 'success' && result.data) {
                ...
                throw new Error(result.message || 'Failed to generate bio');
            }
        } catch (error) {
        ...
        <div className="space-y-6">
            <Tabs defaultValue="profile">
                ...
                <TabsContent value="profile" className="mt-8">
                    {isLoading ? (
                        showTimeout ? (
                            <LoadingTimeout onRetry={refetch} />
                        ) : (
                            <ProfileSkeleton />
                        )
                    ) : error ? (
                        <ErrorState
                            ...
                            message={error}
                            onRetry={refetch}
                            variant={error.includes('timeout') ? 'timeout' : error.includes('connection') ? 'network' : 'default'}
                        />
                    ) : (
                        <DataGrid columns={3}>
