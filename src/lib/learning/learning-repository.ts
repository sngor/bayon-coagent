 ...
 * Consolidates all learning-related database operations
 */
...
export class LearningRepository {
    ...
    private getLearningProgressKeys(userId: string, moduleId: string) {
        return {
            PK: `USER#${userId}`,
            SK: `LEARNING_PROGRESS#${moduleId}`,
        };
    }
    ...
    async saveLearningProgress(userId: string, moduleId: string, completed: boolean): Promise<void> {
        const progressId = `progress-${moduleId}`;
        const progress = {
            id: progressId,
            userId,
            moduleId,
        ...
        const keys = this.getLearningProgressKeys(userId, moduleId);
        await this.repository.put({
            PK: keys.PK,
            SK: keys.SK,
            ...
            UpdatedAt: Date.now(),
            GSI1PK: `USER#${userId}`,
            GSI1SK: `LEARNING_PROGRESS#${progress.lastAccessedAt}`,
        });
    }

    async getUserLearningProgress(userId: string, limit = 100): Promise<any[]> {
        const pk = `USER#${userId}`;
        const results = await this.repository.query(pk, 'LEARNING_PROGRESS#', {
            limit,
            scanIndexForward: false,
        });
        return results.items;
    ...
    async issueCertificate(
        ...
        await this.repository.put({
            PK: keys.PK,
            SK: keys.SK,
            EntityType: 'LearningCertificate',
            Data: certificate,
            CreatedAt: Date.now(),
            UpdatedAt: Date.now(),
            GSI1PK: `USER#${userId}`,
