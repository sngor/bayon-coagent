...
import { generateLearningPlanAction, saveLearningPlanAction } from '@/app/learning-actions';
import { useToast } from '@/hooks/use-toast';
import { useUser } from '@/aws/auth';
import { cn } from '@/lib/utils/common';


export interface AILearningPlanProps {
    className?: string;
}
...
export function AILearningPlan({ className }: AILearningPlanProps = {}) {
    ...
    const handleGenerate = async () => {
        ...
        try {
            const result = await generateLearningPlanAction(challenge);

            if (result.errors) {
                console.error('Learning plan errors:', result.errors);
                toast({
                ...
                toast({
                    title: 'Generation Failed',
                    description: 'No learning plan was generated. Please try again.',
                    variant: 'destructive',
                });
            }
        } catch (error) {
    ...
    const handleSave = async () => {
        ...
        if (!user) {
            toast({
                ...
                description: 'You must be logged in to save learning plans.',
                variant: 'destructive',
            });
            return;
        }
        ...
        setIsSaving(true);
        try {
            const result = await saveLearningPlanAction(challenge, plan, user.id);
...
<head>
    <meta charset="UTF-8">
    <title>Learning Plan - ${challenge.substring(0, 50)}</title>
    <style>
...
<body>
    ...
    ${plan}
        ...
        a.href = url;
        a.download = `learning-plan-${Date.now()}.html`;
        ...
        toast({
            title: 'Downloaded',
            description: 'Learning plan downloaded as HTML file.',
        });
    };
                    ...
                    <CardHeader>
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <CardTitle className="font-headline text-2xl">AI Learning Plan Generator</CardTitle>
                                    <CardDescription>
                                        Describe your challenge or growth area, and get a personalized, actionable learning plan powered by AI
                                    </CardDescription>
                                </div>
                            </div>
                        </div>
                                ...
                                <div className="relative overflow-hidden rounded-xl border-2 border-primary/20 bg-gradient-to-br from-secondary/50 via-secondary/30 to-background p-8">
                                    ...
                                    <div className="relative">
                                        <div className="flex items-center gap-3 mb-6 pb-4 border-b border-primary/10">
                                            ...
                                            <h3 className="font-headline font-semibold text-xl">Your Personalized Learning Plan</h3>
                                        </div>
                                        <div
